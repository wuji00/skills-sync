...
function Read-Pid {
    if (Test-Path $BridgePidFile) { return (Get-Content $BridgePidFile -Raw).Trim() }
    return $null
}
...
switch ($Command) {
    ...
    'stop' {
        ...
        if ($svc -and $svc.Status -eq 'Running') {
            ...
            if (Test-Path $BridgePidFile) { Remove-Item $BridgePidFile -Force }
        } else {
            $bridgeProcId = Read-Pid
            if (-not $bridgeProcId) { Write-Host "No bridge running"; exit 0 }
            if (Test-PidAlive $bridgeProcId) {
                Stop-Process -Id ([int]$bridgeProcId) -Force
                Write-Host "Bridge stopped"
            } else {
    ...
    'status' {
        ...
        if ($bridgeProcId -and (Test-PidAlive $bridgeProcId)) {
            Write-Host "Bridge process is running (PID: $bridgeProcId)"
